...
def main(cfg):
    ...
    # if not os.path.exists(os.path.join(DATA_SRC, 'loops/')):
    #     os.makedirs(os.path.join(DATA_SRC, 'loops/0/'), exist_ok=True)
    #     for file in [f for f in os.listdir(DATA_SRC) if f.endswith(FILE_FORMAT)]:
    #         shutil.copyfile(os.path.join(DATA_SRC, file), os.path.join(DATA_SRC, f'loops/0/{file}'))
    # cfg.dataset.data_src = os.path.join(DATA_SRC, 'loops/0/')
    # # print(cfg.dataset.data_src)
    ...
    # # initiate
    # lst_problematic_tiles = pipeline.segment()
    # lst_problematic_tiles = ["color_grp_full_tile_12.laz", "color_grp_full_tile_4.laz", "color_grp_full_tile_7.laz"]
    # lst_files = [f for f in os.listdir(cfg.dataset.data_src) if f.endswith(FILE_FORMAT) and f not in lst_problematic_tiles]

    # #   _create csv for files referencing
    ...
    # # save file
    # df_split_data.to_csv(os.path.join(cfg.dataset.data_src, 'data_split_metadata.csv'), sep=',', index=False)
    ...
    for loop in range(NUM_LOOPS):
        ...
        loop_tiles_state = {
            "names": list_tiles_names,
            "num_loop": list(loop * np.ones((len(list_tiles_names), 1))),
            "is_problematic": [int(x in pipeline.lst_problematic_tiles) for x in list_tiles_names],
            "is_empty": list(np.zeros((len(list_tiles_names), 1))),
            "num_predictions": list(np.zeros((len(list_tiles_names), 1))),
            "num_garbage": list(np.zeros((len(list_tiles_names), 1))),
            "num_multi": list(np.zeros((len(list_tiles_names), 1))),
            "num_single": list(np.zeros((len(list_tiles_names), 1))),
        }
        pipeline.tiles_state = pd.concat([pipeline.tiles_state, pd.DataFrame(loop_tiles_state)], axis=0)

        # segment
        lst_problematic_tiles = pipeline.segment(verbose=True)
        lst_files = [f for f in os.listdir(cfg.dataset.data_src) if f.endswith(FILE_FORMAT) and f not in lst_problematic_tiles]
